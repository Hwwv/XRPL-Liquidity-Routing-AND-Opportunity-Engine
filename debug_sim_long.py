...
for step in range(1000):
    ...
    if step < 5 or step % 100 == 0:
        print(f'Step {step+1}: TRADE {used}({amt:.2f}) → {path_str}, output={choice.output_amount:.2f}')
    ...
    if step < 5 or step % 100 == 0:
        print(f'  Portfolio: { {str(k): round(v, 2) for k, v in portfolio.items() if v > 0} }, total={total:.2f}')
...
print(f'Final portfolio: { {str(k): round(v, 2) for k, v in portfolio.items() if v > 0} }')
