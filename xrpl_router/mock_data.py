...
def get_mock_graph(limit_per_book: int = BOOK_DEPTH) -> dict[Asset, list[MarketEdge]]:
    """
...
_MOCK_BOOKS_ARBITRAGE: dict[tuple[Asset, Asset], list[tuple[float, float]]] = {
    (Asset("A", None), Asset("B", "")): [(2.0, 10000.0)],
    (Asset("B", ""), Asset("C", None)): [(1.1, 10000.0)],
    (Asset("C", None), Asset("A", None)): [(0.5, 10000.0)],
}
